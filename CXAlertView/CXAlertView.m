...
+ (void)showBackground
{
	if (!__cx_alert_background_window) {
		...
		if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
			CGFloat offset = abs(adoptedSize.width - adoptedSize.height);
			CGFloat maxWidth = MAX(adoptedSize.width, adoptedSize.height) + offset;
			backgroundFrame = CGRectMake(-offset, -offset, maxWidth, maxWidth);
		} else {
			backgroundFrame = CGRectMake(0, 0, adoptedSize.width, adoptedSize.height);
		}
